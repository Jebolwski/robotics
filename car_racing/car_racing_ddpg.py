...
# DDPG modelini tanımla
model = DDPG("CnnPolicy", env, verbose=1, tensorboard_log="./ddpg_carracing_tensorboard/", buffer_size=100000)
...
def train_and_log(model, timesteps):
    ...
    while episode_done_count < 4:  # 4 episod tamamlanana kadar eğitime devam et
        ...
        # 2. Yöntem: Aksiyonları ortamın aksiyon uzayına göre sınırla (normalleştir)
...
plt.xlabel('Episode')
plt.ylabel('Reward')
plt.title('DDPG Training on CarRacing-v2')
